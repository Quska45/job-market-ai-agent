...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--notify", action="append", choices=["console", "discord"], default=["console"])
...
def main() -> None:
    ...
    _run([sys.executable, "apps/collector/report_quality.py", str(output_json), "--write-back"])
    ...
    _run([sys.executable, "apps/reporter/export_jobs.py", str(output_json), "--format", args.report_format])
    summary = build_notification_summary(_load_jobs(output_json))
    sent_channels = notify(summary, args.notify)
